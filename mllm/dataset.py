...
class NLLBLanguageStream(IterableDataset):
    ...
    def __iter__(self):
        ds = load_dataset("allenai/nllb", self.pair_config, split=self.split, streaming=True)
        if self.shuffle_buffer_size is not None:
            stream_seed = self.seed
            if stream_seed is None:
                stream_seed = random.SystemRandom().randrange(2**32)
            ds = ds.shuffle(buffer_size=self.shuffle_buffer_size, seed=stream_seed)
...
class BalancedNLLBDataset(IterableDataset):
    def __init__(
        ...
        for pair_config, lang in zip(pair_configs, langs):
            self.streams_by_lang[lang] = NLLBLanguageStream(
                pair_config,
                lang,
